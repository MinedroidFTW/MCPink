...
namespace MCPink
{
    class CmdHeartbeat : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            try
            {
                Heartbeat.Pump(Beat.MCPink);
            }
            catch (Exception e)
            {
                Server.s.Log("Error with MCPink pump.");
                Server.ErrorLog(e);
            }
        ...
        {
            Player.SendMessage(p, "/heartbeat - Forces a pump for the MCPink heartbeat.  DEBUG PURPOSES ONLY.");
        }
    }
}
