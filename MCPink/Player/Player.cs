/*
	Copyright 2010 MCSharp team (Modified for use with MCZall/MCPink) Licensed under the
    ...
    public sealed class Player
    {
        ...
        public Player(Socket s)
        {
            try
            {
                ...
                Server.s.Log(ip + " joined the server.");
                ...
                extraTimer.Elapsed += delegate
                {
                    ...
                    if (players.Count == 1)
                        SendMessage("There is currently &a" + players.Count + " lonely player online.");
                    else
        ...
        void HandleLogin(byte[] message)
        {
            try
            {
                ...
                if (connections.Count >= 5) { Kick("Sorry there is too many connected players for you to join."); return; }
                ...
                if (Player.players.Count >= Server.players && ip != "127.0.0.1") { Kick("The server is full, sorry."); return; }
                ...
                if (Server.verify)
                {
                        ...
                        .Replace("-", "").ToLower().TrimStart('0'))
                    {
                        ...
                        {
                            Kick("You have failed to login, please try again!"); return;
                        }
                    }
                }
            ...
            if (playerDb.Rows.Count == 0)
            {
                ...
                SendMessage("Welcome to " + Server.name + "! This is your first visit.");
            ...
            if (group.Permission == LevelPermission.Guest)
            {
                ...
                if (Server.antiTunnel)
                {
                    if (!ignoreGrief)
                    {
                        ...
                        {
                            SendMessage("STOP! Your not aloud to dig any further!");
                            SendBlockchange(x, y, z, b); return;
                        }
                    }
                }
        ...
        public void HandlePortal(Player p, ushort x, ushort y, ushort z, byte b)
        {
            ...
            catch { Player.SendMessage(p, "This portal has no Exit."); return; }
        }
            ...
            try
            {
                ...
                if (text != "/afk")
                {
                    if (Server.afkset.Contains(this.name))
                    {
                        ...
                        Player.GlobalMessage("-" + this.color + this.name + Server.DefaultColor + "- is no longer away.");
                        IRCBot.Say(this.name + " is no longer away.");
                    }
                }
                ...
                if (muted) { this.SendMessage("You have been muted."); return; }  //Muted: Only allow commands
        ...
        public void HandleCommand(string cmd, string message)
        {
            try
            {
                ...
                if (jailed) { SendMessage("Your jailed, no commands for you!"); return; }
                if (cmd.ToLower() == "care") { SendMessage("Corneria now loves you with all his heart."); return; }
                if (cmd.ToLower() == "facepalm") { SendMessage("The bot army just simultaneously facepalm'd at your use of this command."); return; }
            ...
            try
            {
                ...
                if (loggedIn)
                {
                    ...
                    if (kickString == "Disconnected." || kickString.IndexOf("Server shutdown") != -1 || kickString == Server.customShutdownMessage)
                    {
                        ...
                        IRCBot.Say(name + " just left the game.");
                ...
                {
                    connections.Remove(this);
                    Server.s.Log(ip + " just left the game.");
                }
            }
            catch (Exception e) { Server.ErrorLog(e); }
        }
        ...
        bool CheckBlockSpam()
        {
            if (spamBlockLog.Count >= spamBlockCount)
            {
                ...
                if (spamTimer < spamBlockTimer && !ignoreGrief)
                {
                    ...
                    Server.s.Log(name + " was kicked for block spam (" + spamBlockCount + " blocks in " + spamTimer + " seconds) Woah!");
                    return true;
                }
            }
